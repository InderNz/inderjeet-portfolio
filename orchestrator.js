...
async function createSubtask(parentKey, summary, tcId) {
  ...
  // Jira returns { id, key, self } at top level
  if (!res.key) log(`[WARN] createSubtask got no key back: ${JSON.stringify(res).slice(0,200)}`);
  return res.key || null;
}
...
async function runStage2(ticket) {
  ...
  stage2DoneAt.set(key, new Date().toISOString());
}
...
const stage2DoneAt = new Map();   // key → ISO string when Stage 2 finished
...
async function pollForApproval() {
  try {
    ...
    for (const issue of issues) {
      ...
      const doneAt = stage2DoneAt.get(issue.key) || new Date(0).toISOString();
      const hasApprove = comments.some(c => {
        if (c.created < doneAt) return false;  // ignore comments before Stage 2
